...
export default function GalleryManager() {
    ...
    const fetchImages = async () => {
        setLoading(true);
        try {
            const res = await axios.get(`${BACKEND}/api/gallery`);
            setImages(res.data || []);
        } catch {
            setImages([]);
        } finally {
    ...
    const handleSave = async (e) => {
        ...
        try {
            await axios.post(`${BACKEND}/api/gallery`, { title, imageUrl });
            await fetchImages();
            setTitle('');
            setImageUrl('');
            setImagePreview('');
